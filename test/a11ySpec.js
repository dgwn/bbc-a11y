...
describe("a11y", function () {
  ...
  it("tests the DOM", function () {
    ...
    $("body").append(
      '<div role="main"><h1>Hello!</h1><p>Some content</p></div>'
    );
  ...
  it("hides errors", function () {
    return a11y
      .test({ hide: ['Found 0 elements with role="main".'] })
      .then(function (outcome) {
        var resultsWithErrors = outcome.results.filter(function (
          standardResult
        ) {
          return standardResult.hiddenErrors.length > 0;
        });
        expect(resultsWithErrors[0].errors).to.eql([]);
        expect(resultsWithErrors[0].hiddenErrors).to.eql([
  ...
  it("hides warnings", function () {
    ...
    return a11y
      .test({ hide: ["First heading was not a main heading"] })
      .then(function (outcome) {
        var resultsWithWarnings = outcome.results.filter(function (
          standardResult
        ) {
          return standardResult.hiddenWarnings.length > 0;
        });
        expect(resultsWithWarnings[0].warnings).to.eql([]);
        expect(resultsWithWarnings[0].hiddenWarnings[0][0]).to.eql(
          "First heading was not a main heading:"
        );
      });
  });
  ...
  it("only runs specific standards", function () {
    return a11y
      .test({
        only: ["Structure: Containers and landmarks: Exactly one main landmark"]
      })
      .then(function (outcome) {
        expect(outcome.results.length).to.equal(1);
        expect(outcome.results[0].standard.name).to.equal(
